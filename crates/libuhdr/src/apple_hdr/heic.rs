...
pub struct Heic {
    #[debug(skip)]
    sdr_image: FloatImageContent,
    hdr_metadata: AppleHdrMetadata,
    #[debug(skip)]
    gainmap: FloatImageContent,
    ...
    pub fn new_from_bytes(heic_bytes: &[u8]) -> Result<Self, String> {
        ...
        // Check whether the image's metadata containers the `HDRGainMapVersion` key.
        {
            ...
            let version_str = node
                .text()
                .ok_or_else(|| "HDRGainMapVersion element has no text content".to_string())?;
        ...
        let (hdr_metadata, exif_data_block) = {
            ...
            let (maker33, maker48) = {
                ...
                let tiff_header_offset =
                    u32::from_be_bytes(exif_bytes[0..4].try_into().unwrap()) as usize;
                ...
                cursor
                    .seek(SeekFrom::Start(exif_ifd_offset as u64))
                    .map_err(|e| format!("Failed to seek to Exif Sub-IFD: {}", e))?;
                let exif_ifd = tiff::TiffIfd::new(
                    &mut cursor,
                    exif_tiff.header.endianness,
                    exif_tiff.header.version,
                )
                .map_err(|e| format!("Failed to parse Exif Sub-IFD: {}", e))?;

                // Find MakerNote (tag 0x927C) in Exif Sub-IFD.
                let makernote_bytes = exif_ifd
                    .entry_with_tag(0x927C)
            ...
            let hdr_metadata = AppleHdrMetadata { headroom };
        ...
        let gainmap_image = lib_heif
            .decode(&gainmap_handle, ColorSpace::Monochrome, None)
        ...
        let gainmap = {
            ...
            if gainmap_plane.bits_per_pixel != 8 {
                return Err(format!(
                    "Expected gain map to have 8 bits per pixel, but got {}",
                    gainmap_plane.bits_per_pixel
                ));
            }

            let mut float_image_content = FloatImageContent::with_extent(
                gainmap_plane.width as usize,
                gainmap_plane.height as usize,
            );
            for y in 0..gainmap_plane.height {
                ...
                for x in 0..gainmap_plane.width {
                    ...
                    float_image_content.set_at(
                        x as usize,
                        y as usize,
                        FloatPixel::new(value, value, value),
                    );
                }
            }
        ...
        let sdr_plane = {
            ...
            let Some(rgba_plane) = planes.interleaved else {
                return Err(
                    "Decoded primary image does not have an interleaved RGBA plane".to_string(),
                );
            };
            rgba_plane
        };
        ...
        let sdr_image = {
            let mut float_image_content =
                FloatImageContent::with_extent(sdr_plane.width as usize, sdr_plane.height as usize);
...
fn detect_color_gamut(handle: &libheif_rs::ImageHandle) -> Result<ColorGamut, String> {
    ...
    if let Some(nclx) = handle.color_profile_nclx() {
        let primaries = crate::colorspace::ColorGamut::from_xy_primaries(
            nclx.color_primary_red_x() as f64,
            nclx.color_primary_red_y() as f64,
            nclx.color_primary_green_x() as f64,
            nclx.color_primary_green_y() as f64,
            nclx.color_primary_blue_x() as f64,
            nclx.color_primary_blue_y() as f64,
            nclx.color_primary_white_x() as f64,
            nclx.color_primary_white_y() as f64,
        );
        return Ok(primaries);
    }
...
fn parse_apple_makernote(
    ...
    for _ in 0..entry_count {
        ...
        let field_type = endianness
            .read_u16(&mut cursor)
            .map_err(|e| format!("MakerNote: {}", e))?;
        let count = endianness
            .read_u32(&mut cursor)
            .map_err(|e| format!("MakerNote: {}", e))?;
        ...
        if (tag == 0x0021 || tag == 0x0030) && field_type == SRATIONAL && count == 1 {
            ...
            let numerator = endianness
                .read_u32(&mut cursor)
                .map_err(|e| format!("MakerNote: {}", e))? as i32;
            let denominator = endianness
                .read_u32(&mut cursor)
                .map_err(|e| format!("MakerNote: {}", e))? as i32;
            ...
            let value = if denominator != 0 {
                ...
                return Err(format!(
                    "MakerNote tag 0x{:04X}: zero denominator in SRATIONAL",
                    tag
                ));
            };
    ...
    let maker33 =
        maker33.ok_or_else(|| "Apple MakerNote: tag 0x0021 (HDRHeadroom) not found".to_string())?;
    let maker48 =
        maker48.ok_or_else(|| "Apple MakerNote: tag 0x0030 (HDRGain) not found".to_string())?;
...
mod tests {
    ...
    fn test_read_heic() {
        ...
        let mut file = File::open(test_dir_path.join("greyhounds-looking-for-a-table.heic"))
            .expect("Failed to open HEIC file");
        let mut heic_bytes = Vec::new();
        file.read_to_end(&mut heic_bytes)
